...
export class RoomFoodViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http
        .get<RoomFood[]>(
          'http://localhost:8081/activities/room-food-services',
          this.httpOptions
        )
        .subscribe((roomFood) => {
    ...
    if(!this.isAdmin){
      this.http
        ...
        .subscribe((id) => {
          ...
          this.http.get<any[]>(
            'http://localhost:8081/activities/room-foods-service/filter/' + this.basicId,
            this.httpOptions
          )
          .subscribe((roomFood) => {
            this.roomFoodsList = roomFood; console.log(roomFood);
  ...
  updateList(){
    this.http.get<RoomFood[]>('http://localhost:8081/activities/room-food-services', this.httpOptions).subscribe(roomFood => {
