...
export class UsersListComponent implements OnInit {
  ...
  deleteBasic(id){
    this.http.delete<UserClient>('http://localhost:8081/users/basics/' + id, this.httpOptions)
    .subscribe((basicUser) => {this.basicUserList = [...this.basicUserList, basicUser]; });
  }

  deletePremium(id){
    this.http.delete<UserClient>('http://localhost:8081/users/premiums/' + id, this.httpOptions)
