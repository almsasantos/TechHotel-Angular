...
export class UsersViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http
    .get<UserClient[]>(
      'http://localhost:8081/users/basics',
      this.httpOptions
    )
    .subscribe((basicUser) => {
      ...
      this.http
    .get<UserClient[]>(
      'http://localhost:8081/users/premiums',
      this.httpOptions
    )
  ...
  updateList(){
    this.http.get<UserClient[]>('http://localhost:8081/users/basics', this.httpOptions).subscribe(basicUser => {
      this.basicUserList = [...basicUser];
    }, () => console.log("updateList"));

    this.http.get<UserClient[]>('http://localhost:8081/users/premiums', this.httpOptions).subscribe(premiumUser => {
